 ...
 * 
 * 在其他语言中，由花括号封闭的代码块都有自己的作用域（就是 JavaScript 中的执行环境），因而支持根据条件来定义变量，
 * 而在 JavaScript 中，if 语句中的变量声明会将变量添加到当前的执行环境中。在使用 for 语句时尤其要牢记这一差异。
 */
 ...
 *
 * 很多现代语言都推荐尽可能延迟声明变量，而在 JavaScript 中却是糟糕的建议，因为它缺少块级作用域。
 * 所以，最好的做法是在函数体的顶部声明函数中可能用到的所有变量。
 */
